...
if [[ " ${usersArray[@]:1} " =~ " $usernameSQL " ]]; then
	...
	if [ "$?" -eq 0 ]; then #If password check successful
		...
		if ! [ -z "${dbChoice}" ]; then #DB Select condition
			...
			SavePath=$(zenity --file-selection --title="ProjectBot Selection" --text "Select file to be imported to \n table '$tableChoice' in database '$dbChoice' at '$serverSelection' " --save --confirm-overwrite )
			mkdir $SavePath
			echo $SavePath
			SaveFile="$SavePath/`date +%d-%m-%Y::%H:%M`-$dbChoice.sql"
			echo $SaveFile
			if ! [ -z "${SavePath}" ]; then #File Select condition
				...
				if [[ " ${sudoArray[@]} " =~ " $usernameSQL " ]]; then #Main DevOps check
					...
					statusQuery=Done
					result=$(mysqldump --host=$SQLserverIP $connectionCode1 $connectionCode2 --port=3306 $dbChoice > $SaveFile 2>&1 )
...
mysql --host=$SQLserverIP  --user=grantor --password=YRzJNPQqmYm8795aGdHFR47d6h --port=3306 -e "INSERT INTO admin_logger.exportTable (GrantorName, RequestorName, FileName, DatabaseName, TableName, RequestedSourceIP, RequestedServer, StatusSQLQuery) VALUES ('$SuperSQLAuthUser', '$usernameSQL', '$SavePath', '$dbChoice', '$tableChoice', '$serverSelection' , '$machineIPValue' , '$statusQuery');"
